...
# ── FloatSlider: horizontal slider + spinbox in one row ──────────────────────────
class FloatSlider(QWidget):
    ...
    def __init__(self, label, mn, mx, step, value):
        ...
        dec = max(0, min(4, len(f"{self._step:.4f}".rstrip("0").split(".")[-1])))
...
class ImageLabel(QLabel):
    ...
    def mousePressEvent(self, e):
        if e.button() == Qt.MouseButton.LeftButton:
            held = getattr(self.window(), '_held_key', None)
            if held in ('B', 'W', 'N'):
                rc = self._pos_to_proxy(e.position().toPoint())
                if rc is not None:
                    self.pointClicked.emit(rc[0], rc[1], held)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("neg-invert  |  film-strip inverter")
        ...
        btn_row = QHBoxLayout()
        for b in (self._btn_open, self._btn_clear, self._btn_flip, self._btn_rot):
            btn_row.addWidget(b)
        ...
        self._btn_reset_bw = QPushButton("Reset BP/WP")
        self._btn_mode     = QPushButton(self._mode_label(D_MODE))
        self._btn_reset.clicked.connect(self._on_reset)
        self._btn_reset_bw.clicked.connect(self._on_reset_bw)
        self._btn_mode.clicked.connect(self._on_cycle_mode)

        ctrl_btn_row = QHBoxLayout()
        for b in (self._btn_reset, self._btn_reset_bw, self._btn_mode):
    ...
    def _set_active(self, idx):
        ...
        self._btn_mode.setText(self._mode_label(st.settings.mode))
        self._sync_sliders()
    ...
    def _mode_label(mode):
        return {'log': 'Mode: LOG', 'lin': 'Mode: LIN', 'psl': 'Mode: PSL'}[mode]

    def _on_cycle_mode(self):
        cycle = {'lin': 'psl', 'psl': 'log', 'log': 'lin'}
        self._current_mode = cycle[self._current_mode]
        self._btn_mode.setText(self._mode_label(self._current_mode))
        st = self._st()
        if st is None:
            return
        s = st.settings
        s.mode    = self._current_mode
    ...
    def _on_reset(self):
        ...
        old = st.settings
        self._current_mode = D_MODE
        self._btn_mode.setText(self._mode_label(D_MODE))
    ...
    # ── Save (Ctrl+S) — full-resolution, identical pipeline ──────────────────────
    def _on_save(self):
        st = self._st()
        ...
        out = apply_temperature_tint(out, s.temperature, s.tint)
        out = apply_saturation(out, 1.0 + s.saturation)
        out = _apply_flip_rot(out, s.flip, s.rot)
        try:
            path = save_processed_image(out, st.path, st.src_max, suffix='_inv')
